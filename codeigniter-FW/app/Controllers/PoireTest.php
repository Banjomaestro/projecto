...
class PoireTest extends BaseController
{
    public function index()
    {
       ...
       // $modelID->deleteIDModel('hola');
        ...
        //on crée un nouvel identifiant
        $modelID = model(IdModel::class);
        $modelID->subscribe('joseph',md5('hola'));
        ...
        //On vérifie que ça écrase bien le ID_rep précédent
        ...
        echo "/ Vérification getAnInternauteQuizzByQuestion() :";
        echo "\n";

        $quizzQ4=$modelQI->getAnInternauteQuizzByQuestion('joseph','4');

        echo $quizzQ4['ID_ident'];
        echo $quizzQ4['ID_rep'];

        echo "\n"; 


        echo "/ Vérification getAllInternauteQuizz() :";
        echo "\n";

        $quizzJoseph=$modelQI->getAllInternauteQuizz('Joseph');

        foreach($quizzJoseph as $id)
        {
            foreach($id as $value){
                echo $value;
            };
        ...
        $modelQI->deleteInternauteResponse('joseph','19');
